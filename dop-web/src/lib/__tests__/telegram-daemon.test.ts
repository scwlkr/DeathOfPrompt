...
// The Telegram daemon now delegates to the shared engine (dop-engine.ts via
...
// inside dop.ts, so we still mock it to isolate tests from a running Ollama.
...
describe('F2: Telegram daemon delegates to shared engine', () => {
  it('chatWithAgent routes to processChatSync with telegram-<chatId> session', async () => {
    ...
    expect(mockProcessChatSync).toHaveBeenCalledWith('telegram-12345', 'Hi', 'default');
  });
  ...
  it('chatWithAgent falls back to telegram-global when no chatId given', async () => {
    ...
    expect(mockProcessChatSync).toHaveBeenCalledWith('telegram-global', 'hi', 'default');
  });
