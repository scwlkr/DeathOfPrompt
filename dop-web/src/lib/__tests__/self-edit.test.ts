...
// Scratch file under dop-web/ so resolveInsideRepo accepts it.
const SCRATCH = path.join(REPO_ROOT, 'dop-web', 'src', 'lib', '__tests__', '.scratch-self-edit.txt');
const SCRATCH_REL = 'dop-web/src/lib/__tests__/.scratch-self-edit.txt';
...
describe('self-edit: marker parsing', () => {
  it('parses READ_FILE markers', () => {
    const edits = parseSelfEdits('Let me check [[READ_FILE: dop-web/.env]] first.');
    expect(edits).toHaveLength(1);
    expect(edits[0]).toEqual({ kind: 'read', path: 'dop-web/.env' });
  });
...
Restart to pick it up.`;
  ...
  it('parses WRITE_FILE blocks', () => {
    const reply = `[[WRITE_FILE: dop-web/hello.txt]]
hi there
[[/WRITE_FILE]]`;
    const edits = parseSelfEdits(reply);
    expect(edits).toHaveLength(1);
...
describe('self-edit: path safety', () => {
  ...
  it('rejects writes into node_modules', () => {
    const r = applySelfEdit({ kind: 'write', path: 'dop-web/node_modules/evil.js', content: 'no' });
    expect(r.ok).toBe(false);
  });

  it('rejects .db files', () => {
    const r = applySelfEdit({ kind: 'write', path: 'dop-web/prisma/data/dop.db', content: 'no' });
    expect(r.ok).toBe(false);
  });

  it('rejects writes under data/', () => {
    const r = applySelfEdit({ kind: 'write', path: 'dop-web/data/anything.txt', content: 'no' });
    expect(r.ok).toBe(false);
  });
});
