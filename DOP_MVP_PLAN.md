# DOP MVP(Minimum Viable Product) Plan

## Purpose

This is a plan to create a minimum viable product for Death of Prompt. The goal is to create a working prototype that can be used to test the core concepts of the project.

## Core Concepts

1. DOP's purpose is to make it so that we don't prompt anymore.
2. Prompts need to progress from hopeful wishing on a crystal ball to conversations.
3. DOP is accessed through a telegram bot, and a local web interface.
...
2. A telegram bot that can be used to interact with DOP.
3. A local web interface that can be used to interact with DOP.
